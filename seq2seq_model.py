...
class Seq2SeqModel:
            ...
            if knowledge_dataset is None:
                ...
                if os.path.isdir(knowledge_dataset):
                    ...
                    if index_path:
                        ...
                        if self.args.save_knowledge_dataset:
                            self.dataset.get_index("embeddings").save(
                                os.path.join(knowledge_dataset,
                    ...
                    if self.args.save_knowledge_dataset:
                        self.dataset.get_index("embeddings").save(
                            os.path.join(
                                self.args.output_dir, "knowledge_dataset", "hf_dataset_index.faiss")
                        )
            ...
            if encoder_decoder_type in ["bart", "mbart", "marian"]:
                ...
                if encoder_decoder_name:
                    ...
                    self.model = EncoderDecoderModel.from_encoder_decoder_pretrained(
                        os.path.join(encoder_decoder_name, "encoder"), os.path.join(
                            encoder_decoder_name, "decoder")
                    )
                    self.encoder_tokenizer = tokenizer_class.from_pretrained(
    ...
    def train(
        ...
        if args.model_name and os.path.exists(args.model_name):
            try:
                ...
                logger.info("   Will skip the first %d steps in the current epoch",
        ...
        for current_epoch in train_iterator:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        self.save_model(output_dir_current,
                                        optimizer, scheduler, model=model)
                    ...
                    if args.evaluate_during_training and (
                        ...
                        if args.save_eval_checkpoints:
                            self.save_model(
                                output_dir_current, optimizer, scheduler, model=model, results=results)
            ...
            if args.evaluate_during_training and args.evaluate_each_epoch:
                ...
                report.to_csv(os.path.join(args.output_dir,
                              "training_progress_scores.csv"), index=False)
    ...
    def eval_model(self, eval_data, output_dir=None, verbose=True, silent=False, **kwargs):
        ...
        result = self.evaluate(eval_dataset, output_dir,
                               verbose=verbose, silent=silent, **kwargs)
    ...
    def predict(self, to_predict):
        ...
        for batch in tqdm(
            ...
            if self.args.model_type == "marian":
                ...
                question_hidden_states = self.model.question_encoder(input_ids)[
                    0]
                ...
                doc_scores = torch.bmm(
                    ...
                    docs_dict["retrieved_doc_embeds"].float(
            ...
            if self.args.model_type in ["bart", "marian"]:
                ...
                outputs = self.model.generate(
                    context_input_ids=docs_dict["context_input_ids"].to(
                        self.device),
                ...
                retrieved_docs = [
                    doc for doc in self.retriever.index.get_doc_dicts(docs_dict["doc_ids"])]
            else:
                outputs = self.model.generate(
                    input_ids=input_ids,
        ...
        if self.args.model_type in ["rag-token", "rag-sequence"]:
            ...
            if self.args.multiprocessing_chunksize == -1:
                chunksize = max(len(all_outputs) //
                                (self.args.process_count * 2), 500)
            else:
    ...
    def save_model(self, output_dir=None, optimizer=None, scheduler=None, model=None, results=None, dataset=None):
        ...
        if model and not self.args.no_save:
            ...
            if self.args.model_type in ["bart", "mbart", "marian", "rag-token", "rag-sequence"]:
                ...
                model_to_save = (
                    self.model.encoder.module if hasattr(
                ...
                model_to_save = (
                    self.model.decoder.module if hasattr(
