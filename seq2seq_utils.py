...
if transformers.__version__ < "4.2.0":
    def shift_tokens_right(input_ids, pad_token_id, decoder_start_token_id): return _shift_tokens_right(
...
def preprocess_batch_for_hf_dataset(dataset, encoder_tokenizer, decoder_tokenizer, args):
    if args.model_type == "bart":
        ...
        decoder_input_ids = shift_tokens_right(
            decoder_input_ids, encoder_tokenizer.pad_token_id, encoder_tokenizer.lang_code_to_id[
                args.tgt_lang]
        )
...
class Seq2SeqDataset(Dataset):
    def __init__(self, encoder_tokenizer, decoder_tokenizer, args, data, mode):
        cached_features_file = os.path.join(
            args.cache_dir, args.model_name.replace(
                "/", "_") + "_cached_" + str(args.max_seq_length) + str(len(data))
        )
        ...
        if os.path.exists(cached_features_file) and (
            ...
            if (mode == "train" and args.use_multiprocessing) or (
                ...
                with Pool(args.process_count) as p:
                    self.examples = list(
                        tqdm(p.imap(preprocess_data, data, chunksize=chunksize), total=len(
                            data), disable=args.silent,)
                    )
            else:
                self.examples = [preprocess_data(
                    d) for d in tqdm(data, disable=args.silent)]
    ...
    def __init__(self, tokenizer, args, data, mode):
        ...
        if os.path.exists(cached_features_file) and (
            ...
            if (mode == "train" and args.use_multiprocessing) or (
                ...
                with Pool(args.process_count) as p:
                    self.examples = list(
                        tqdm(p.imap(preprocess_fn, data, chunksize=chunksize),
                             total=len(data), disable=args.silent,)
                    )
            else:
                self.examples = [preprocess_fn(
                    d) for d in tqdm(data, disable=args.silent)]
...
def generate_faiss_index_dataset(data, ctx_encoder_name, args, device):
    ...
    dataset = dataset.map(
        partial(split_documents, split_text_n=args.split_text_n,
    ...
    ctx_encoder = DPRContextEncoder.from_pretrained(
        ctx_encoder_name).to(device=device)
    ...
    dataset = dataset.map(
        partial(embed, ctx_encoder=ctx_encoder,
                ctx_tokenizer=ctx_tokenizer, device=device),
        batched=True,
    ...
    if args.save_knowledge_dataset:
        output_dataset_directory = os.path.join(
            args.output_dir, "knowledge_dataset")
