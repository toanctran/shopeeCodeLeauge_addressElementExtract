...
class ModelArgs:
    ...
    def get_args_for_saving(self):
        args_for_saving = {key: value for key, value in asdict(
            self).items() if key not in self.not_saved_args}
    ...
    def save(self, output_dir):
        ...
        with open(os.path.join(output_dir, "model_args.json"), "w") as f:
            ...
            if args_dict["tokenizer_type"] is not None and not isinstance(args_dict["tokenizer_type"], str):
                args_dict["tokenizer_type"] = type(
                    args_dict["tokenizer_type"]).__name__
